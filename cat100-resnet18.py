...
program_name = "100-batch32-lr0.01-SGD"
'''
# unzip datafile
import zipfile
...
model = ResNet(ResidualBlock, CLASS_FINE)
model.cuda()
...
def test(model, test_loader):
    ...
    result.to_csv('samplesummission2.csv', index=False, sep=',')


if __name__ == "__main__":
    # train
    # train(model, train_fineloader, valid_fineloader, start_epoch)
    # load trained model
    params = torch.load(save_path)
    model.load_state_dict(params['model'])
    # test
